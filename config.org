...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Change it like below so mu4e moves sent messages into your sent folder. 
  ;; Unlike Gmail, Office 365 doesn't do this automatically.
...
#+END_SRC
*** Preferences
**** Quit mu4e without confirmation
#+BEGIN_SRC emacs-lisp
...
**** Instructions on creating authinfo.gps in [[Method1][Password Management]]
#+BEGIN_SRC emacs-lisp
(require 'smtpmail)
...
#+END_SRC
*** Headers to show
 The headers to show in the headers list -- a pair of a field and its
 width, with `nil' meaning 'unlimited' (better only use that for the
 last field.  These are the defaults:
#+BEGIN_SRC emacs-lisp
